...
function getResizeQuery() {
    if (props.resizeMaxW && props.resizeMaxH) {
        return `?width=${props.resizeMaxW}&height=${props.resizeMaxH}`;
    } else return "";
}
</script>

<template>
    <img v-if="fileTypes[file.type].startsWith('image')" :src="'/media/'+file.id+'.'+file.type+getResizeQuery()" loading="lazy" class="media" :style="{maxWidth: maxW, maxHeight: maxH}" @click="emit('click');">
        
    <div v-if="fileTypes[file.type].startsWith('video') && !videoLoaded" class="relative" 
        @click.stop="videoLoaded = true;">
        <img :src="'/media/'+file.id+'.'+file.type+'.jpg'+getResizeQuery()" loading="lazy" class="media" :style="{maxWidth: maxW, maxHeight: maxH}" ref="videoThumb" @load="videoSize = {width: videoThumb.width, height: videoThumb.height}">
